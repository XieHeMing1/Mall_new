...
import com.kedie.serialportlibrary.thread.SerialPortReadThread;
...
public class SerialPortManager extends SerialPort {
    ...
    public boolean openSerialPort(File device, int baudRate, int parity, int dataBits, int stopBit) {
        this.deviceFile = device.getAbsolutePath();
                ...
                if (null != mFileOutputStream && null != sendBytes && 0 < sendBytes.length) {
                    try {
                        ...
                        if (null != mOnSerialPortDataListener) {
                            mOnSerialPortDataListener.onDataSent(sendBytes,deviceFile);
                        }
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
    ...
    private void startReadThread() {
        mSerialPortReadThread = new SerialPortReadThread(mFileInputStream) {
            ...
            public void onDataReceived(byte[] bytes) {
                if (null != mOnSerialPortDataListener) {
                    mOnSerialPortDataListener.onDataReceived(bytes,deviceFile);
                }
            }
        };
